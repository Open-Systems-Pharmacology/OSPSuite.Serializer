...
namespace OSPSuite.Serializer.Attributes
{
   public class EnumAttributeMapper<TEnum, TContext> : AttributeMapper<TEnum, TContext>
   {
      ...
      public override object ConvertFrom(string attributeValue, TContext context) => Enum.Parse(typeof(TEnum), attributeValue);

      public override string Convert(TEnum valueToConvert, TContext context) => valueToConvert.ToString();
   }
}